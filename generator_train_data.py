...
def add_text_to_image(image, text, font_path, font_size, position, text_color, outline_color=(255, 255, 255),
                      outline_width=3):
    draw = ImageDraw.Draw(image)
...
def generate_images(n, width, height, text_length, font_path, font_size_range, path_to_save, mangas):
    ...
    for i in tqdm(range(n)):
        ...
        random_texts = [generate_random_japanese_text(random.randint(0, text_length)) for i in range(0, 3)]
        ...
        for y in range(len(random_texts)):
            position = (random.randint(-font_size * 3, width - font_size * 2), random.randint(0 - font_size * 2, height))

            text_size = (font_size * len(random_texts[y]), font_size + 10)

            add_rectangle_to_image(black_image, position, text_size, 255)

            add_text_to_image(
                noise_image, random_texts[y], font_path, font_size, position,
                (text_color, text_color, text_color), outline_color, outline_width=random.randint(0, 4)
            )
...
def main():
    ...
    parser.add_argument("--font_path", type=str, default="./NotoSansJP-Bold.ttf",
                        help="Путь к файлу шрифта (по умолчанию: ./NotoSansJP-Bold.ttf).")
    ...
    parser.add_argument("--path_to_save", type=str, default="./train_data/",
                        help="Путь для сохранения изображений (по умолчанию: ./DeepEraser/train_data/).")
    parser.add_argument("--manga_path", type=str, default="./clm",
                        help="Путь к изображениям манги (по умолчанию: ./clm).")
